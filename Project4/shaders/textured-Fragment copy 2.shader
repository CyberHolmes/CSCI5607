...
void main() {
  ...
  vec3 lightDir_p = normalize(lights - pos);
  float dist = distance(pos, lights);
  float attenuation = 20.0/(dist*dist);
  vec3 diffuseC = color*max(dot(lightDir_p,normal),0.0);
  vec3 viewDir = normalize(-pos); //We know the eye is at (0,0)!
  vec3 reflectDir = reflect(viewDir,normal);
  float spec = max(dot(reflectDir,-lightDir_p),0.0);
  if (dot(lightDir_p,normal) <= 0.0) spec = 0;
