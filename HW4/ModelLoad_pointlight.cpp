...
const GLchar* fragmentSource =
  ...
  "   if (dot(lightDir,normal) <= 0.0) spec = 0;"
  "   vec3 specC = vec3(1.0,1.0,1.0)*pow(spec,4);"
  "   outColor = vec4((diffuseC+specC) * attenuation +ambC, 1.0);"
  "}";
...
int main(int argc, char *argv[]){
  ...
  glm::vec3 lpos = glm::vec3(2,2,2);
  int inLightPosLoc = glGetUniformLocation(shaderProgram, "inLightPos");
  assert(inLightPosLoc != -1);
  glUniform3f(inLightPosLoc,lpos.x,lpos.y,lpos.z);
  ...
  float posr=0, negr=0, posup=0, negup=0;
  ...
  srand(time(NULL));
    ...
    // lpos = glm::vec3(rotationM * glm::vec4(lpos, 1.0));
    // glUniform3f(inLightDirLoc,ldir.x,lpos.y,ldir.z);
