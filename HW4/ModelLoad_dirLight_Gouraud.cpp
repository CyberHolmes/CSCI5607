...
glm::vec3 iniColor = glm::vec3(0.f,0.7f,0.f);

char window_title[] = "DirectionLight + Gouraud";
...
int main(int argc, char *argv[]){
  ...
  //Read command line paramaters to get scene file
   if (argc < 2){
      sceneFileName = "./models/teapot.txt";
   } else {
    argv += 1, argc -= 1;
    sceneFileName = argv[0];
    argv += 1, argc -= 1;
   }
  ...
  int inColorLocation = glGetUniformLocation(shaderProgram, "inColor");
  assert(inColorLocation != -1);
  glUniform3f(inColorLocation,iniColor.x,iniColor.y,iniColor.z);

  int inLightDirLoc = glGetUniformLocation(shaderProgram, "inLightDir");
  assert(inLightDirLoc != -1);
  glUniform3f(inLightDirLoc,1,1,1);
  ...
  // position
  // glm::vec3 position = glm::vec3( 0, 0, 3.0 );
  glm::vec3 iniPos = glm::vec3( -0.007975,-2.528682,1.533058 );
  glm::vec3 position = iniPos;//glm::vec3( -0.007975,-2.528682,1.533058 );
  // horizontal angle : toward -Z
  float iniHAngle = 3.137401f;//3.14f;
  float horizontalAngle = iniHAngle;//3.14f;
  // vertical angle : 0, look at the horizon
  float iniVAngle = 1.027500f;//0.0f;
  float verticalAngle = iniVAngle;//0.0f;
  ...
  glm::vec3 ldir0 = glm::normalize(glm::vec3(1,1,1));
  glm::vec3 ldir = ldir0;
  srand(time(NULL));

	while (!quit){
    double currentTime = SDL_GetTicks();
    float deltaTime = SDL_GetTicks()/1000.f; //float(currentTime - lastTime);
    // printf("deltatime=%f\n",deltaTime);
    ...
    while (SDL_PollEvent(&windowEvent)){
      ...
      if (windowEvent.type == SDL_KEYUP && windowEvent.key.keysym.sym == SDLK_q) quit = true;
      if (windowEvent.type == SDL_KEYUP && windowEvent.key.keysym.sym == SDLK_a) position += right * deltaTime * speed;
      if (windowEvent.type == SDL_KEYUP && windowEvent.key.keysym.sym == SDLK_d) position -= right * deltaTime * speed;
      // if (windowEvent.type == SDL_KEYUP && windowEvent.key.keysym.sym == SDLK_u) position -= direction * deltaTime * speed;
      // if (windowEvent.type == SDL_KEYUP && windowEvent.key.keysym.sym == SDLK_o) position += direction * deltaTime * speed;
      if (windowEvent.type == SDL_KEYUP && windowEvent.key.keysym.sym == SDLK_w) position -= up * deltaTime * speed;
      if (windowEvent.type == SDL_KEYUP && windowEvent.key.keysym.sym == SDLK_s) position += up * deltaTime * speed;
      if (windowEvent.type == SDL_KEYUP && windowEvent.key.keysym.sym == SDLK_SPACE) RotateLight = !RotateLight;
      if (windowEvent.type == SDL_KEYUP && windowEvent.key.keysym.sym == SDLK_m) RotateObject = !RotateObject;
      if (windowEvent.type == SDL_KEYUP && windowEvent.key.keysym.sym == SDLK_r) {
        
        position = iniPos;
        horizontalAngle = iniHAngle;
        verticalAngle = iniVAngle;
        FoV = iniFoV;
        ldir = ldir0;
      ...
      if (windowEvent.type == SDL_KEYUP && windowEvent.key.keysym.sym == SDLK_c)
        glUniform3f(inColorLocation,rand()%100/100.0,rand()%100/100.0,rand()%100/100.0);

      if (windowEvent.type == SDL_MOUSEWHEEL)//scroll up
      {
        FoV -= 200.0 * (float)windowEvent.wheel.y/(float)screen_height;
        // FoV -= 100.0 * (float)windowEvent.wheel.x/(float)screen_width;
        FoV = (FoV<20)?20:(FoV>75)?75:FoV;
    ...
    if (SDL_GetMouseState(&mx, &my) & SDL_BUTTON(SDL_BUTTON_LEFT)) { //Is the mouse down?
        ...
        horizontalAngle += mouseSpeed * deltaTime * float(screen_width/2 - mx );
        verticalAngle   += mouseSpeed * deltaTime * float( screen_height/2 - my );
        // horizontalAngle += mouseSpeed * deltaTime * (1/2.0 - float(mx/screen_width ));
        // verticalAngle   += mouseSpeed * deltaTime * (1/2.0 - float(my/screen_height ));
        verticalAngle = fmin( maxAngleInRadians, fmax( -maxAngleInRadians, verticalAngle) );
        // direction = glm::vec3 (
        //     cos(verticalAngle) * sin(horizontalAngle),
        //     sin(verticalAngle),
        //     cos(verticalAngle) * cos(horizontalAngle)
        ...
        SDL_WarpMouseInWindow(window,screen_width/2, screen_height/2); //reset mouse position
    } 
    mouse_dragging = true;
    ...
    if (RotateLight){
      ...
      rotationM = glm::rotate(rotationM,float(0.07 * 3.14f/2),up);
      // rotationM = glm::rotate(rotationM,float(0.07 * 3.14f/2),glm::vec3(0.0f, 1.0f, 1.0f));
      // rotationM = glm::rotate(rotationM,float(0.07 * 3.14f/2),glm::vec3(1.0f, 0.0f, 0.0f));
      ldir = glm::vec3(rotationM * glm::vec4(ldir, 1.0));
      glUniform3f(inLightDirLoc,ldir.x,ldir.y,ldir.z);
    }
    ...
    glm::mat4 view = glm::lookAt(
      ...
      position + direction,  //Look at point
      up); //Up
    GLint uniView = glGetUniformLocation(shaderProgram, "view");
    glUniformMatrix4fv(uniView, 1, GL_FALSE, glm::value_ptr(view));
    
    // glm::mat4 proj = glm::perspective(3.14f/4, aspect, 1.0f, 10.0f); //FOV, aspect, near, far
    glm::mat4 proj = glm::perspective(glm::radians(FoV), aspect, 1.0f, 10.0f); //FOV, aspect, near, far
