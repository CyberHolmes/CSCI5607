...
Sphere* spheres= new Sphere[100]; //allocate space for 100 spheres, will change it to more flexible container later
int numPlights = 0;
PointLight* plights = new PointLight[10];
int numSlights = 0;
SpotLight* slights = new SpotLight[10];
int numDlights = 0;
DirectionalLight* dlights= new DirectionalLight[10];
AmbientLight alight = AmbientLight(Color());
...
void parseSceneFile(std::string fileName){
  ...
  while (fgets(line, MAX_LEN, fp)){
    ...
    if (!strcmp(command, "material:")){ //material: .75 .75 .75 .75 .75 .75 .3 .3 .3 32 .2 .2 .2 1.5
      ...
      m->set(ar,ag,ab,dr,dg,db,sr,sg,sb,ns,tr,tg,tb,ior);
    }
    if (!strcmp(command, "sphere:")){ //sphere: -3 1 0 0.7
      float x,y,z,r;
      sscanf(line, "sphere: %f %f %f %f",&x,&y,&z,&r);
      vec3 v=vec3(x,y,z);
      spheres[numSpheres]=Sphere(v,r,*m);
      numSpheres++;       
    }
    ...
    if (!strcmp(command, "spot_light:")){ //sphere: -3 1 0 0.7
      ...
      slights[numSlights]=SpotLight(Color(r,g,b),vec3(px,py,pz),vec3(dx,dy,dz),a1,a2);
      numSlights++;    
    }
